...
# Using cursor
@app.route('/students_using_cursor')
def students_using_cursor():
    conn = db.engine.raw_connection()  # Get raw connection
    cursor = conn.cursor()  # Create a cursor
    cursor.execute("SELECT * FROM student")  # Execute the SQL query
    students = cursor.fetchall()  # Fetch all the results
    cursor.close()  # Close the cursor
    conn.close()  # Close the connection

    return render_template('students_cursor.html', students=students)


# Calling stored procedure
@app.route('/student_by_roll', methods=['POST', 'GET'])
def student_by_roll():
    if request.method == "POST":
        rollno = request.form.get('rollno')
        # Call the stored procedure
        result = db.engine.execute(f"CALL GetStudentByRoll('{rollno}')")
        students = result.fetchall()  # Fetch the result from the procedure
        return render_template('studentdetails.html', query=students)
    return render_template('search_by_roll.html')


# Calling MySQL function
@app.route('/get_student_name/<rollno>')
def get_student_name(rollno):
    # Call the MySQL function in a query
    result = db.engine.execute(f"SELECT GetStudentName('{rollno}') as sname")
    student_name = result.fetchone()[0]  # Fetch the student name
